...
class kereta{ 
    ...
    void viewPostOrder(struct DataPohonAVL *roott);
    void viewInOrder(struct DataPohonAVL *roott);
    void viewPreOrder(struct DataPohonAVL *roott);
    void cetakPostOrder(struct DataPohonAVL *roott);
    void cetakInOrder(struct DataPohonAVL *roott);
    void cetakPreOrder(struct DataPohonAVL *roott);
    void tujuanCITLINK(); 
    void jadwalCITLINK();
	...
	private:
		...
		int harga, saldo, tanggalpem, jumlahpen, jumlahpeng, idpenum, height; // variabel transaksi
    ...
    string namapen, idpen, tanggalpemm;
		string kelaskereta[3] = {"Ekonomi", "Eksekutif", "Bisnis"}; // materi 2 :: ArrayStatis
    string tujuankereta[4] = {"Jakarta", "Surabaya", "Bandung", "Semarang"}; 
    string nmkereta[5] = {"BINRIZDA", "CEPATIID", "GOKERETA", "MARINAIK", "SUPERKER"}; 
...
struct DataPohonAVL* newDataPohonAVL(int bb, string aa, string cc, string dd, string ee, string ff, string gg, string hh){
	struct DataPohonAVL* nodee = (struct DataPohonAVL*)malloc(sizeof(struct DataPohonAVL));
	nodee->bb = bb;
  nodee->aa = aa;   
  nodee->cc = cc;   
  nodee->dd = dd;
...
struct DataPohonAVL *rightRotate(struct DataPohonAVL *y){
	struct DataPohonAVL *x = y->kirii;
	struct DataPohonAVL *T2 = x->kanann;

	x->kanann = y;
	y->kirii = T2;

	y->height = max(height(y->kirii), height(y->kanann))+1;
	x->height = max(height(x->kirii), height(x->kanann))+1;

	return x;
}

struct DataPohonAVL *leftRotate(struct DataPohonAVL *x){
	struct DataPohonAVL *y = x->kanann;
	struct DataPohonAVL *T2 = y->kirii;
...
int getBalance(struct DataPohonAVL *N){
	if (N == NULL)
		return 0;
	return height(N->kirii) - height(N->kanann);
}

struct DataPohonAVL* insert(struct DataPohonAVL* nodee, int bb, string aa, string cc, string dd, string ee, string ff, string gg, string hh){
	if (nodee == NULL){
		return(newDataPohonAVL(bb, aa, cc, dd, ee, ff, gg, hh));
	}
	if (bb < nodee->bb){
		nodee->kirii = insert(nodee->kirii, bb, aa, cc, dd, ee, ff, gg, hh);
	}
  else if (bb > nodee->bb){
		nodee->kanann = insert(nodee->kanann, bb, aa, cc, dd, ee, ff, gg, hh);
	}
  else{
		return nodee;
  }
	nodee->height = 1 + max(height(nodee->kirii), height(nodee->kanann));

	int balance = getBalance(nodee);

	if (balance > 1 && bb < nodee->kirii->bb){
		return rightRotate(nodee);
	}
	if (balance < -1 && bb > nodee->kanann->bb){
		return leftRotate(nodee);
	}
	if (balance > 1 && bb > nodee->kirii->bb){
		nodee->kirii = leftRotate(nodee->kirii);
		return rightRotate(nodee);
	}
	if (balance < -1 && bb < nodee->kanann->bb){
		nodee->kanann = rightRotate(nodee->kanann);
		return leftRotate(nodee);
	}
	return nodee;
}

struct DataPohonAVL * minValueDataPohonAVL(struct DataPohonAVL* nodee){
  struct DataPohonAVL* currentt = nodee;
  while (currentt->kirii != NULL)
    currentt = currentt->kirii;
  return currentt;
}

void kereta::viewPreOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
    cout << endl;
		cout << " ID Penumpang (nim) : " << roott->bb << endl;
		cout << " Nama Penumpang     : " << roott->aa << endl;  
		kereta::viewPreOrder(roott->kirii);
		kereta::viewPreOrder(roott->kanann);
	}
}

void kereta::viewInOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
		kereta::viewInOrder(roott->kirii);
		cout << endl;
		cout << " ID Penumpang (nim) : " << roott->bb << endl;
		cout << " Nama Penumpang     : " << roott->aa << endl;  
		kereta::viewInOrder(roott->kanann);
	}
}

void kereta::viewPostOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
		kereta::viewPostOrder(roott->kirii);
		kereta::viewPostOrder(roott->kanann);
		cout << endl;
		cout << " ID Penumpang (nim) : " << roott->bb << endl;
		cout << " Nama Penumpang     : " << roott->aa << endl;  
	}
...
void kereta::cetakPreOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
    cout << endl;
    cout << "==============================================================" << endl;
	  cout << " KERETA API                                     BOARDING PASS" << endl;
	  cout << "--------------------------------------------------------------" << endl;
    cout << " Nama / Name                    Kodebooking / Bookingcode" << endl;
	  cout << " " << roott->aa << "                 00XXX" << endl;
	  cout << " Nomor Identitas / Id Number    Tipe Penumpang / Pax Type" << endl;
	  cout << " " << roott->bb << "                     Ekonomis" << endl;
	  cout << " Kereta Api / Train             No Tempat Duduk / Seat Number" << endl;
	  cout << " CITLINK                        00XXX"  << endl;
	  cout << " Berangkat / Departure          Perkiraan Tiba / Eta" << endl;
	  cout << " Yogyakarta                     " << roott->cc << endl;
	  cout << " " << roott->dd << "-" << roott->ee << "-" << roott->ff;
	  cout << "                     " << roott->dd << "-" << roott->ee;
    cout << "-" << roott->ff << endl;
    cout << " " << roott->gg << "                          " << roott->hh << endl;
	  cout << "--------------------------------------------------------------" << endl;
	  cout << "                            Check In At Yogyakarta ";
	  cout << roott->dd << "-" << roott->ee << "-" << roott->ff << endl;
	  cout << "==============================================================" << endl;  

    ofstream doks("tiket CITLINK.txt", ios::app);
    
    doks << "==============================================================" << endl;
	  doks << " KERETA API                                     BOARDING PASS" << endl;
	  doks << "--------------------------------------------------------------" << endl;
    doks << " Nama / Name                    Kodebooking / Bookingcode" << endl;
	  doks << " " << roott->aa << "                 00XXX" << endl;
	  doks << " Nomor Identitas / Id Number    Tipe Penumpang / Pax Type" << endl;
	  doks << " " << roott->bb << "                     Ekonomis" << endl;
	  doks << " Kereta Api / Train             No Tempat Duduk / Seat Number" << endl;
	  doks << " CITLINK                        00XXX"  << endl;
	  doks << " Berangkat / Departure          Perkiraan Tiba / Eta" << endl;
	  doks << " Yogyakarta                     " << roott->cc << endl;
	  doks << " " << roott->dd << "-" << roott->ee << "-" << roott->ff;
	  doks << "                     " << roott->dd << "-";
    doks << roott->ee << "-" << roott->ff << endl;
    doks << " " << roott->gg << "                          " << roott->hh << endl;
	  doks << "--------------------------------------------------------------" << endl;
	  doks << "                            Check In At Yogyakarta ";
	  doks << roott->dd << "-" << roott->ee << "-" << roott->ff << endl;
	  doks << "==============================================================" << endl;
	  doks << endl; 
		kereta::cetakPreOrder(roott->kirii);
		kereta::cetakPreOrder(roott->kanann);
	}
}

void kereta::cetakInOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
		kereta::cetakInOrder(roott->kirii);
		cout << endl;
    cout << "==============================================================" << endl;
	  cout << " KERETA API                                     BOARDING PASS" << endl;
	  cout << "--------------------------------------------------------------" << endl;
    ...
    doks << " " << roott->gg << "                          " << roott->hh << endl;
	  ...
	  doks << "==============================================================" << endl;
	  doks << endl;  
		kereta::cetakInOrder(roott->kanann);
	}
}

void kereta::cetakPostOrder(struct DataPohonAVL *roott){
	if(roott != NULL){
		kereta::cetakPostOrder(roott->kirii);
		kereta::cetakPostOrder(roott->kanann);
		cout << endl;
    cout << "==============================================================" << endl;
	  cout << " KERETA API                                     BOARDING PASS" << endl;
	  cout << "--------------------------------------------------------------" << endl;
...
void kereta::menu(){
  ...
  cout << " 3. Tiket Kereta Bandara (CITLINK)" << endl;
  cout << " 4. EXIT" << endl;
  cout << "==============================================================" << endl;
  ...
  if (pilmenu==1){
    ...
    do {
      ...
      cout << "|                                                            |" << endl;
      cout << "==============================================================" << endl;
      cout << "                   KERETA BANDARA (CITLINK) " << endl;
      cout << "==============================================================" << endl;
		  cout << "                        DATA PENUMPANG" << endl;
			cout << "--------------------------------------------------------------" << endl;
   		cout << " 1. Tambah Data" << endl;
      ...
      cin >> pilihdata;
        ...
        case 1:
          ...
          roott = insert(roott, idpenum, namapen, tujuan, tanggalpemm, bulanpem, tahunpem, berangkat, tiba);
          cout << endl;
          break;
        case 2:
          kereta::viewInOrder(roott);
          cout << endl;
  			  kereta::kembalimenu2();
          break;
        case 3:
          kereta::viewPreOrder(roott);
          cout << endl;
  			  kereta::kembalimenu2();
          break;
        case 4:
          kereta::viewPostOrder(roott);
          cout << endl;
  			  kereta::kembalimenu2();
          break;
        case 5:
          kereta::cetakInOrder(roott);
          cout << endl;
			    cout << "------------------ PEMESANAN TIKET BERHASIL ------------------" << endl;
          cout << endl;
          kereta::kembalimenu();
...
void kereta::tujuanKAJJ(){
  ...
  kereta::deleteAwalSingle(); 
	...
	cin >> piltujuan;
  if (piltujuan==1){
    tujuan = tujuankereta[0];
  }
  else if(piltujuan==2){
    tujuan = tujuankereta[1];
  }
  else if(piltujuan==3){
    tujuan = tujuankereta[2];
  }
  else if(piltujuan==4){
    tujuan = tujuankereta[3];
  }
  else{
    cout << endl;
    cout << endl << " Maaf, Pilihan Yang Anda Masukkan Salah" << endl;
...
void kereta::jadwalKAJJ(){
  ...
  if (piltujuan==1){
    if(piljadwal==1){
      namakereta = nmkereta[0];
      berangkat = "09.00";
      tiba = "13.00";
    }
    else if(piljadwal==2){
      namakereta = nmkereta[1];
      berangkat = "11.00";
      tiba = "15.00";
    }
    else if(piljadwal==3){
      namakereta = nmkereta[2];
      berangkat = "14.00";
      tiba = "18.00";
    }
    else if(piljadwal==4){
      namakereta = nmkereta[3];
      berangkat = "16.00";
      tiba = "20.00";
    }
    else if(piljadwal==5){
      namakereta = nmkereta[4];
      berangkat = "18.00";
      tiba = "22.00";
    }
    else{
...
void kereta::tujuanCITLINK(){
  ...
  cout << "--------------------------------------------------------------" << endl;
  cout << " 1. Bandara Adisutjipto                             Rp 10.000   " << endl;
  cout << " 2. Bandara Yogyakarta                              Rp 15.000" << endl;
  cout << "==============================================================" << endl;
	cout << " Pilih Tujuan Keberangkatan (1-2) : ";
	cin >> piltujuan;
	cout << endl;
  if (piltujuan==1){
    tujuan = "Bandara Adisutjipto";
    harga = 10000;
  }
  else if(piltujuan==2){
    tujuan = "Bandara Yogyakarta";
    harga = 15000;
  }
  else{
    cout << endl;
